...
# START MULTIVARIABLE
ukbio_only_selected_snp <- ukbio_all[ukbio_all$rsid %in% dt_all_sig_clump$SNP,]

exp_1 <- brain_1[brain_1$SNP %in% dt_all_sig_clump$SNP, c(2, 6, 7,4,5)]
ukbio_only_selected_snp_ordered <- ukbio_only_selected_snp[order(ukbio_only_selected_snp$rsid), c(4,7,8,5,6)]
...
ukbio_only_selected_snp_ordered <- ukbio_only_selected_snp[order(ukbio_only_selected_snp$rsid), c(4,7,8)]
sum(duplicated(ukbio_only_selected_snp_ordered)) #0
...
ukbio_mult <- rename(ukbio_only_selected_snp_ordered, c("SNP" = "rsid", "outcome.beta" = "beta", "outcome.se" = "se"))
...
ukbio_mult <- as.data.frame(ukbio_mult)


data_frames <- list(ukbio_mult, exp_1_ordered, exp_2_ordered, exp_3_ordered,
...
?mr_mvivw
ukbio_multivariable_result <- data.frame("exposure" = MR_multi@Exposure, 
                                       "outcome" = rep("ukbio", 6),
...
write_xlsx(ukbio_multivariable_result, "/gpfs/gibbs/pi/polimanti/diana/ukbio_multivariableMr.xlsx")
