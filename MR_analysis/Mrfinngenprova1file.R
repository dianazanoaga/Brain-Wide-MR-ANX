...
brain_all <-fread("/gpfs/gibbs/pi/polimanti/diana/files_format/1444.txt")
brain_all <- as.data.frame(brain_all)


i <- "/gpfs/gibbs/pi/polimanti/diana/files_format/1444.txt"
  ...
  merge_brain_out <- merge(x=finn_clump, y=brain_all, by.x= "SNP", by.y = "rsid")
    ...
    merge_brain_out = matrix(c(0,0,0,0), nrow=1, ncol=1)
  }
  ...
  merge_finn_out <- merge(x=brain_clump, y=finn_all, by.x= "SNP", by.y = "rsids")
    ...
    merge_finn_out = matrix(c(0,0,0,0), nrow=1, ncol=1)
  }

  dim(merge_finn_out)[1]
  ...
  write_xlsx(list_data, file= "/gpfs/gibbs/pi/polimanti/diana/MR_results/finngen/3446.xlsx")
